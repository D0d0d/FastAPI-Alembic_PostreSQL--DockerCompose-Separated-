...
@router.get('/', response_model=schemas.ListProducts)
def get_products(db: Session = Depends(get_db), limit: int = 10, page: int = 1,
                 search: str = ''):
    skip = (page - 1) * limit

    products = db.query(models.Product).group_by(models.Product.id).filter(
        models.Product.name.contains(search)).limit(limit).offset(skip).all()
    return {'status': 'success', 'results': len(products),
            'products': products}


@router.post('/', status_code=status.HTTP_201_CREATED,
             response_model=schemas.ProductResponse)
def create_product(product: schemas.CreateProduct,
...
def update_product(id: str, product: schemas.UpdateProduct,
                   db: Session = Depends(get_db)):
